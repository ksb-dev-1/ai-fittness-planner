...
// 1. Correct class name from the @google/generative-ai package
...
export async function generateTrainingPlan(
  ...
  // 2. Correct initialization for standard SDK
  const model = genAI.getGenerativeModel({
    model: "gemini-2.5-flash",
    systemInstruction:
      "You are an expert fitness trainer. You must respond with valid JSON only. Do not include markdown or reasoning.",
  });
  ...
  try {
    ...
    // 3. Correct way to extract text in this SDK
    ...
    console.error("[Gemini AI] Error generating training plan:", error);
    throw error;
  }
}

// Ensure your helper functions (buildPrompt, formatPlanResponse) are exported or
// present in the same file as before.
